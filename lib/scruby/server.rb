...
module Scruby
  ...
  class Server
    ...
    DEFAULTS = { buffers: 1024, control_buses: 4096, audio_buses: 128, audio_outputs: 8, audio_inputs: 8,
                 host: 'localhost', port: 57_111, path: '/Applications/SuperCollider/scsynth' }.freeze
    ...
    def boot
      ...
      sleep 0.01 until ready || !@thread.alive? || (Time.now > timeout)
    ...
    def send(message, *args)
      message = Message.new message, *args unless (Message === message) || (Bundle === message)
      @client.send message
    end
