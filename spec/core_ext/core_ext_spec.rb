...
describe Proc do
  describe '#arguments' do
    it do
      proc {}.should respond_to(:arguments)
    end

    it 'should get empty array if proc has no args' do
      proc {}.arguments.should eql([])
    end

    it 'should get one argument name' do
      proc { |arg| }.arguments.should eql([:arg])
    end

    it 'should get arg names with several args' do
      proc { |arg, arg2, arg3| }.arguments.should eql(%i[arg arg2 arg3])
    end
  end
end
...
describe Array, 'monkey patches' do
  describe '#collect_with_index' do
    ...
    it 'should wrap and zip' do
      %i[a b c].wrap_and_zip([1]).flatten.should == [:a, 1, :b, 1, :c, 1]
