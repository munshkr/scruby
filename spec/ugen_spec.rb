...
describe Ugen do
  ...
  describe 'initialization and inputs' do
    ...
    it 'should not accept non valid inputs' do
      -> { @ugen = MockUgen.new(:audio, 'hola') }.should raise_error(ArgumentError)
    end

    it 'should require at least one argument' do
      -> { MockUgen.new }.should raise_error(ArgumentError)
    end

    it 'should be a defined rate as the first argument' do
      -> { MockUgen.new(:not_a_rate, 1) }.should raise_error(ArgumentError)
    end

    it 'should use the highest rate when passing an array' do
      MockUgen.new(%i[audio control], 1).rate.should == :audio
    end

    it 'should be a defined rate as array' do
      -> { MockUgen.new(%i[not_a_rate audio], 1) }.should raise_error(ArgumentError)
    end
