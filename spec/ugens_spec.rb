...
describe Ugens do
  before do
    @udefs = YAML.safe_load(File.open("#{File.dirname __FILE__}/../lib/scruby/ugens/ugen_defs.yaml"))
  end

  it 'should define Ugen classes' do
    @udefs.each_pair { |key, _val| eval(key).should_not be_nil }
  end

  it 'each ugen should be Ugen subclass' do
    @udefs.each_pair { |key, _val| eval(key).superclass.should eql(Scruby::Ugens::Ugen) }
  end
  ...
  it 'should raise argumen error if not passed required' do
    Gendy1.stub!(:new)
    -> { Gendy1.ar }.should raise_error(ArgumentError)
  end

  it 'should not accept more than the required arguments' do
    -> { SinOsc.ar(1, 2, 3, 4, 5, 6) }.should raise_error(ArgumentError)
  end
